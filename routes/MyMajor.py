...
def mymajor():
    ...
    must_last_year = a.year if (a := db.session.query(
        Must.year).distinct().order_by(Must.year.desc()).first()) else 0
    year = last_year if "year" not in request.args else int(
        request.args.get("year"))
    standard = must_last_year if "standard" not in request.args else int(
        request.args.get("standard"))
    ...
    result = result.outerjoin(
        Must,
        db.and_(
            Must.year == standard, Must.sid == Major.sid,
    ...
    result = result.all()
    result = [
        i if i[3] else (i[0], i[1], i[2], findNearestMust(i[0], year))
        for i in result
    ]
